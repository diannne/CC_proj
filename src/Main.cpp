...
int main(int argc, char * argv[]) {
    ...
    if (processingType == GOOCH) {
        ...
        if (argc == 4) {
            char * charpointer = new char[1000];
            if (convertToChar(argv[3], &charpointer) == false) {
                printf("Cannot perform conversion to char.\n");
                return -2;
            }
            char * pEnd = NULL;
            long int iterations = strtol(charpointer, &pEnd, 10);
            if (iterations == 0L) {
                printf("Invalid third argument for Pitie Color Grading.\n");
                printf("Provide a valid interations number or accept the default value 10.\n");
                return -2;
            }
