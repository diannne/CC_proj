...
void MatrixModule::matrix_copy(float * toCopy, const float * fromCopy, 
	unsigned int size) {
	for (unsigned int i = 0; i < size; ++i) {
		toCopy[i] = fromCopy[i];
	}
...
void MatrixModule::matrix_gram(float *a, unsigned int size,
	...
	for (int k = 0; k < size; k++) {
		...
		for (int i = 0; i < size; i++) {
			...
			matrix_access(q, size, size, i, k) = 
				matrix_access(a, size, size, i, k)/matrix_access(r, size, size, k, k);
		}
		...
		for (int j = k + 1; j < size; j++) {
			...
			for (int i = 0; i < size; i++) {
				//r[k][j] += q[i][k] * a[i][j];
				matrix_access(r, size, size, k, j) += 
					matrix_access(q, size, size, i, k)*matrix_access(a, size, size, i, j);
			}
