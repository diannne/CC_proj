...
//===========================================================================
#ifdef _UNICODE
inline FIBITMAP* FreeImage_Load_Wrapper(FREE_IMAGE_FORMAT fif, const char* fileName, int flags = 0)
{
	return FreeImage_LoadU(fif, fileName, flags);
}

inline FREE_IMAGE_FORMAT FreeImage_GetFIFFromFilename_Wrapper(const char* fileName)
{
	return FreeImage_GetFIFFromFilenameU(fileName);
}

inline FREE_IMAGE_FORMAT FreeImage_GetFileType_Wrapper(const char* fileName, int size = 0)
{
	return FreeImage_GetFileTypeU(fileName, size);
}

inline BOOL FreeImage_Save_Wrapper(FREE_IMAGE_FORMAT fif, FIBITMAP *dib, const char* fileName, int flags = 0)
{
